...
class RightLayout(GridLayout):
    ...
    def show_data(self, filename):
        ...
        for x in range(-1, -15, -1):
            ...
            for x in range(-1, -15, -1):
                if str((np.array(data.iloc[x].values))[18]) == " ":
                    self.val10 = str(data1[18])                                         # motor current (from BMS)
                else:
                    self.val10 = str((float(np.array(data.iloc[x].values)[18]))*i)               
                    self.num10 = self.fit_to_scale(self.val110, 0, 400)
                    try:
                        self.val110 = round(float((np.array(data.iloc[x].values)[18]))*i)
                    except:
                        pass
                    break
            '''
