...
i = 10 # σχέση μετάδοσης (μπαταρία-κινητήρας)
    ...
    def show_data(self, filename):
        ...
        for x in range(-1, -15, -1):
            if str((np.array(data.iloc[x].values))[20]) == " ":
                ...
                self.val3 = str(random.randint(38,40))#str((np.array(data.iloc[-1].values)[21])) # battery temperature (from BMS)
                ...
                self.val3 = str(random.randint(38,40))#str(np.array(data.iloc[x].values)[21]) # battery temperature (from BMS)
    ...
    def show_data(self, filename):
        ...
        for x in range(-1, -15, -1):
            if str((np.array(data.iloc[x].values))[8]) == " ":
                ...
                self.val9 = str(random.randint(46,48))#str((np.array(data.iloc[-1].values)[21])) # battery temperature (from BMS)
                self.val10 = str(np.array(data.iloc[-1].values)[13])  # Motor Current (from VESC)
                self.val11 = str(random.randint(51,53))#str((np.array(data.iloc[-1].values)[21])) # battery temperature (from BMS)

            else:
                self.val8 = str(np.array(data.iloc[x].values)[8])
                self.val9 = str(random.randint(46,48))#str((np.array(data.iloc[-1].values)[21])) # battery temperature (from BMS)
                self.val10 = str(np.array(data.iloc[x].values)[13])
                self.val11 = str(random.randint(52,53))#str((np.array(data.iloc[-1].values)[21])) # battery temperature (from BMS)
